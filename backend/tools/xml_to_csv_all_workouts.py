...
class HealthDataExtractor(object):
    ...
    def open_for_writing(self):
        self.handles = {}
        self.paths = []
        path = os.path.join(self.directory, 'All_Workouts.csv')
        f = open(path, 'w')
        f.write(','.join(FIELDS) + '\n')
        self.handles['All_Workouts'] = f
    ...
    def write_records(self):
        for node in self.nodes:
            if node.tag == 'Workout':
                ...
                self.handles['All_Workouts'].write(line)
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print('USAGE: python xml_to_csv_all_workouts.py /path/to/export_07172018.xml',
              file=sys.stderr)
        sys.exit(1)
